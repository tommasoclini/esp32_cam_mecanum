...
esp_err_t jpg_httpd_handler(httpd_req_t *req) {
  ...
  ESP_LOGI(TAG, "JPG: %uKB %ums", ((unsigned int)(fb_len / 1024)),
           ((unsigned int)((fr_end - fr_start) / 1000)));
  return res;
}

static const httpd_uri_t camera_uri = {.handler = jpg_httpd_handler,
                                       .method = HTTP_GET,
                                       .uri = "/camera",
                                       .user_ctx = NULL};
...
static const httpd_uri_t camera_stream_uri = {.handler =
